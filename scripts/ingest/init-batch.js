 ...
 *   - 0.6.9 (issue #25 fix, PR-C):新增 `normalizeFileEntry(raw)` 函数 ——
 *     接受 `path` / `source_path` / `file_path` / `file` 4 种路径字段名别名
 *     (SKILL.md 步骤 3 文档字段名 ↔ 脚本字段名漂移,导致 v0.6.9 ERR_INVALID_ARG_TYPE 回归);
...
/**
 * v0.6.9 (issue #25 fix, PR-C):把 LLM 在 batch.json 中写的"任意写法"统一归一为脚本权威字段。
 *
 * 接受多种字段名别名(优先级 = 排序):
...
async function main() {
  ...
  //
  // v0.6.9 (issue #25 fix, PR-C):`normalizeFileEntry` 把 LLM 任意写法
