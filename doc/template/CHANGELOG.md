...
## v0.5.9(2026-09-09)—— source 页强加"自由追加节"占位骨架

**触发**:用户反馈 `ai-engineering-skills-map-2026.md` 实例只有"重点摘录/我的思考/总结"3 节,LLM 没有按文章独特结构呈现,模板"限制太大"。

**核心变更**:`page-source.md` 在 `## 重点摘录` 之前新增占位骨架 `## 阅读路线`(LLM 必改:改节名 / 拆多节 / 极短文豁免一句话说明);lint 加 C21(违规 WARN)。

### 变更清单

#### 1. `page-source.md`

- [ ] 在 LLM 阅读须知块新增"⚠️ 自由追加节强约束(v0.5.9 起)"小节,明确"## 重点摘录 之前必须至少有 1 个 H2 节(lint C21 WARN)"+ 占位骨架节名 `## 阅读路线` 默认值 + 改名建议清单 + 极短源文件豁免说明 + 强制溯源
- [ ] 在阅读须知块下方、`## 重点摘录` 上方新增 `## 阅读路线` 占位骨架节(LLM 必改)
- [ ] 阅读须知块底部对齐行更新:`C20` → `C20 + C21`

#### 2. `template/README.md`

- [ ] 3 节骨架硬约束表下新增一行"自由追加节(占位骨架 ## 阅读路线,v0.5.9 起)"

#### 3. 后续衍生

- [ ] `scripts/ingest/lint-stub.js`:新增 C21(`## 重点摘录` 之前 H2 节数 = 0 → WARN)
- [ ] `scripts/gen-page.js`:为 `## 阅读路线` 占位节输出明确的改造指引文案
- [ ] `skills/aeps-llm-wiki-ingest/SKILL.md` 步骤 7:加"v0.5.9 起强 prompt",要求 LLM 读完源文件后先问"这篇有什么独特结构"再写正文
