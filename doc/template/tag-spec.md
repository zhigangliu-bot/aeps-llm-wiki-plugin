# tag-spec — aeps-llm-wiki-plugin 全局 tag 字典(6 轴)

> **状态**:v1.0 冻结版(2026-09-02)
> **权威性**(scope of authority):本文件是 plugin 的 **`tags:` 字段权威字典**,是以下 4 类核心模板填 `tags:` 时的唯一权威来源 —— plugin 维护者直接编辑维护:
> - **[`page-source.md`](page-source.md)** — `type: source` 页(`tags:` 行必须以本字典为准)
> - **[`page-analysis.md`](page-analysis.md)** — `type: analysis` 页(`tags:` 行必须以本字典为准)
...
> **强制校验**:每条 tag 必须以 `domain/` / `layer/` / `phase/` / `docform/` / `maturity/` / `tec/` 之一为前缀(正则 `^(domain|layer|phase|docform|maturity|tec)/[a-z0-9][a-z0-9-]*$`),由 `doc/schema/frontmatter.schema.json` 的 `tags.items.pattern` 硬约束;裸 tag / 状态词 / OKF type 值 / 版本号 / 人名(非 entity 页)FAIL,详见 §1.4 隔离机制 + §8 lint 治理
...
每篇文档选 **6 个左右的不同维度 tag 组合**(硬约束:最少 5 条,最多 10 条;**硬约束**由 `doc/schema/frontmatter.schema.json` 的 `tags.minItems`/`maxItems` 校验,落地为 §8 的 Tag Volume WARN。低于 5 条覆盖度不足、高于 10 条切片区分度下降,均触发 WARN)。
...
| axis         | 词数 | 单值/多值                                                                  | 必填?     |
| ------------ | ---- | -------------------------------------------------------------------------- | --------- |
| `domain`   | 14   | 单值(软上限 ≤ 2,最多 10)                                                  | ✅ 必填   |
| `layer`    | 9    | 单值优先                                                                   | 可选      |
| `phase`    | 9    | **可多值**(需求+验证同时存在常见)                                    | 可选      |
| `docform`  | 14   | **单值**                                                         | ✅ 必填   |
| `maturity` | 5    | **单值**(`concept < research < pilot < production < standard` 等级序) | ⚠️ 推荐 |
| `tec`      | ~40  | 单值优先                                                                   | 可选      |
...
- 不含人名 / 公司名(详见 §1.6.1 实体页人名承担机制)
- 长度 ≤ 2-3 个词(防止过宽)

#### 1.6.1 实体页人名 / 公司名承担机制

**核心规则**:**`tags:` 内不允许出现人名 / 公司名**(无论 entity 页还是非 entity 页)。实体页的"我是谁"由 `aliases:` + 文件名 + 文件路径承担,不进 tag 系统。

| 机制 | 字段 / 形式 | 作用 |
|---|---|---|
| **文件名** | `entities/person/andrej-karpathy.md` | 物理路径 + Obsidian 唯一名解析(`[[andrej-karpathy]]`) |
| **aliases 字段** | frontmatter `aliases: [Andrej Karpathy, Karpathy]` | Obsidian 别名机制,`[[Andrej Karpathy]]` / `[[Karpathy]]` 都解析到本页 |
| **`entities/` 目录** | 物理路径 | 7 个 entity 子类绑死目录(详见 README.md §2.4) |

**示例**(`type: entity.person`):
...
aliases:
  ...
  - "Karpathy"
tags:
  - docform/study-notes    # 形态: 学习笔记
  - domain/ai              # 业务域: AI
  - layer/ai-agent         # 层级: AI Agent 应用层
  - tec/claude             # 技术栈: Claude
---
```
...
title: "Andrej Karpathy"
tags:
  - andrej-karpathy        # ❌ FAIL: 人名进 tag
  - karpathy               # ❌ FAIL: 别名进 tag
---
```

**为什么实体页的"人名"不进 `tags:`**:

1. **6 轴字典设计目的**:`tags:` 服务于"跨切面检索",按 `domain/` / `layer/` / `phase/` / `docform/` / `maturity/` / `tec/` 切片;人名是"实体身份",跟 6 轴正交
2. **唯一性约束**:同一 wiki 里可能多次出现 "Andrej Karpathy"(实体页 + 引用他的 analysis 页 + 引用的 source 页),进 tag 会导致同一关键词污染多页检索
3. **物理路径 + aliases 已经足够**:Obsidian 唯一名 + 别名机制让人名承担"双向链接"职责,跟 tag 系统的"切片检索"职责完全分开
4. **6 轴无 `entity/` axis**:6 轴字典里没有 `entity/xxx` 前缀,人名按 6 轴硬约束也无法表达

**lint 联动**:见 §8 表格 "人名 / 公司名出现在 tag 里(非 entity 页) → FAIL";此规则**对所有 type 一视同仁**,包括 entity 页自身。
...
| **`domain` 软上限 ≤ 2,最多 10**                         | script 不强制,review 时把握                       |
| **OKF type 出现在 tag 里**                                | FAIL(应走`type` 字段)                           |
| **状态词出现在 tag 里**                                   | FAIL(应走`status` 字段)                         |
| **版本号出现在 tag 里**                                   | FAIL(走正文或`stale_after`)                     |
| **人名 / 公司名出现在 tag 里**(所有 type,**包括 entity 页自身**)              | FAIL;走 `aliases:` + 文件名 + `entities/` 目录(详见 §1.6.1)                        |
