...
| v0.1.5 | 2026-09-09 | **解冻** 批次 9 (issue #11-#17 修复):`(1)` `init-batch.js` 接受 `subdir` 作为 `target_subdir` 别名(归一后剔除 subdir 键,issue #15);`(2)` `gen-page.js` frontmatter 注入管道重构:模板 CRLF 归一(块替换正则失效的真实根因,issue #16)+ `--tags` / `--aliases` / `--source-resource` / `--source-title` 新参数 + `DEFAULT_TAGS_BY_TYPE` 字典(14 子类自动注入 ≥5 tags)+ `stale_after` 自动 `generated.at +1y` ISO datetime + `--patch-frontmatter-only` 复用同一管道支持全字段(issue #14/#16/#17/#13-Drift2);`(3)` `build-related-pages.js` sources 类型不符 WARN → ERROR exit 2(issue #12/#17);`(4)` `aggregate-index.js` sentinel 区间重写(`AGGREGATE-START/END` 标记,init 模板同步;无标记存量文件保持追加行为,issue #11 方案 B)。**解冻范围**:§1.1 契约表 init-batch / build-related-pages 行 + §1.3 复用表 gen-page / aggregate-index 条款。 | zhigang.liu(Claude Code 实施 + 复核) |
...
| `scripts/ingest/build-related-pages.js` | `--project <dir> --batch <json>` | **双向反链生成**。读 `batch.json` 的 `files[]` + 扫 `knowledge/sources/<slug>.md` frontmatter + 扫 `knowledge/entities/**` + `knowledge/concepts/**` frontmatter 的 `## 来源资料` 节。对每对(source ↔ entity/concept):① source 页 → 写 `## 相关页面(Related Pages)` 区块(按 Entities / Concepts 分组 wikilink;某组空则省子标题,全空则省整节);② entity/concept 页 → 反向追加 `## 来源资料` 节(按 source title 排序 wikilink;空则省整节)。**每次 ingest 完全重建**,不保留人工条目。**v0.6.5 起(v0.1.5 批次 9,issue #12/#17 fix)**:`sources` 字段类型不符(整字段非数组或元素非对象)从 WARN 升级为 **ERROR exit 2**(对齐 SKILL.md 步骤 19「FAIL 必须修复后才算 ingest 完成」),该页跳过反链写盘,stdout JSON 增 `fail` / `errors_by_file` 字段 | 写文件:`knowledge/sources/*.md` + `knowledge/{entities,concepts}/**/*.md` | 3 fixture:1 source → 2 entity/concept / 多 source 共享同一 entity / 空 source(无抽取) |
...
| 已有脚本 | 是否复用 | 如何复用 |
|---|---|---|
| `scripts/gen-page.js` | ✅ 复用 | SKILL.md 调 `gen-page.js --type source --slug ... --ext ... --converter ... --subdir ... --source-file ...` 生成 source 页骨架;同理 entity/concept 子页。**v0.6.0 起(v0.1.4 批次 6,issue #2 fix)** 加 `--patch-frontmatter-only` flag:SKILL.md 步骤 7 "写完正文后回填 --summary" 改用此 flag,只 patch frontmatter 不动正文;目标文件不存在 → fallback 全量生成(WARN)。**v0.6.5 起(v0.1.5 批次 9,issue #13/#14/#16/#17 fix)**:frontmatter 注入管道统一为「CLI 传入 > 脚本自动推导 > 模板默认」;新增 `--tags` / `--aliases`(逗号分隔)/ `--source-resource` / `--source-title` 参数;entity/concept 页自动注入最小合规 skeleton(`DEFAULT_TAGS_BY_TYPE` 字典 ≥5 tags、`stale_after = generated.at +1y` ISO datetime、aliases/description/summary fallback);模板 CRLF 归一(块替换正则失效根因);`--patch-frontmatter-only` 复用同一管道,支持**全部** frontmatter 字段(含 tags/description/aliases)。 |
