...
> **前端契约(必读)**:`knowledge/` 是给 **Obsidian** 消费的。LLM 写、Obsidian 读、plugin 管一致性。本文件所有约定(链接用 `[[wikilink]]` / tag 走 YAML list / 文件名 `<slug>.md` / 目录即 vault 子文件夹)都必须让 Obsidian 直接打开就能用 —— 无需任何格式转换、映射层、中间文件。Obsidian 不识别 = 本知识库无法被消费,**所有 LLM 操作前必须脑里带这条约束**。
