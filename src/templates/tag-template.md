...
每篇文档选 **3–5 个不同维度的 tag 组合**(超出这个范围会推高维护成本且检索区分度下降)。
...
| **每页 tag 数量 > 5**                                     | WARN,提示"切片失去区分度,建议合并"                |
| **每页 tag 数量 < 3**                                     | WARN,提示"覆盖太薄,建议补"                        |
...
> 实现侧可把这 4 行直接落到 lint 配置文件(`.lint-rules.yaml` / `tag-lint.json`),每条范式在配置里只是一段条件表达式 + 提示模板,新增示例族挂到对应范式下,无需改 lint engine 主体。
