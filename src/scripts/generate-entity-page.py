"""generate-entity-page.py — 写 knowledge/entities/<subtype>/<slug>.md(entity 类型)。

CLI:
    python3 generate-entity-page.py --project-dir . --subtype person --slug <slug> \\
        --meta-json <json> --body-file <path>

行为(DESIGN.md §1.1 + §2.5 + §3.1):
    - 校验 subtype ∈ {person, organization, project, product, event, place, other}(7 合法值)
    - 自动建 knowledge/entities/<subtype>/ 目录(若不存在)
    - 注入 frontmatter:通用 5 必填 + entity-specific aliases
    - 路径:knowledge/entities/<subtype>/<slug>.md
    - 走 Q7 死循环防护(二次写盘用 atomic_write_preserving_mtime)

输出:
    stdout JSON: {"path": "knowledge/entities/<subtype>/<slug>.md", "wrote": true, "size": <N>}
    exit 0 / 1
...
# 7 合法 entity subtype(权威源:src/templates/concept-entities-readme.md §1)
...
def _validate_meta(meta: dict[str, Any], subtype: str) -> None:
    ...
    # type 必须是 entity subtype 之一(且与 --subtype 一致)
    ...
    # entity-specific 必填
    entity_missing = [k for k in ENTITY_REQUIRED if k not in meta]
    if entity_missing:
        raise ValueError(f"entity 类型必填字段缺失:{entity_missing}")

    # aliases 必须是 list(空数组合法)
...
def _build_frontmatter(meta: dict[str, Any], subtype: str) -> str:
    ...
    # aliases(entity-specific)
    lines.append("aliases:")
    for alias in meta["aliases"]:
        lines.append(f"  - {_scalar(alias)}")

    # subtype(辅助字段,便于 OKF reader 识别;非必填但常用)
    if "subtype" in meta:
...
def _target_path(project: Path, subtype: str, slug: str) -> Path:
    """计算目标文件路径:knowledge/entities/<subtype>/<slug>.md。"""
    entity_dir = project / "knowledge" / "entities" / subtype
    entity_dir.mkdir(parents=True, exist_ok=True)
    return entity_dir / f"{slug}.md"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    """主流程入口:写 knowledge/entities/<subtype>/<slug>.md。"""
    ...
    if subtype not in ENTITY_SUBTYPES:
        raise ValueError(
            f"--subtype 非法:{subtype};合法 7 值 = {sorted(ENTITY_SUBTYPES)}"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="generate-entity-page.py",
        description="写 knowledge/entities/<subtype>/<slug>.md(7 合法 subtype + aliases 必填)。",
    )
    parser.add_argument(
        "--project-dir",
        default=".",
    ...
    parser.add_argument(
        "--subtype",
        required=True,
        choices=sorted(ENTITY_SUBTYPES),
        help="entity subtype(7 合法值之一)。",
    )
    parser.add_argument(
        "--slug",
        required=True,
        help="实体 slug(全小写连字符,见 templates/concept-entities-readme.md)。",
    )
    parser.add_argument(
        "--meta-json",
        required=True,
