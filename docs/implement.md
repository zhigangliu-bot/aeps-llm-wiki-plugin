...
- 写入 `<project>/raw/.aeps-plugin-version: 0.5.5`
...
- [ ] **D5** 发 GitHub release:
  ...
  - **CHANGELOG.md 从设计文档提炼**(M4 上架前从 design.md §0 + implement.md §0 现状盘点摘出"v0.5.5 含哪些能力";tag-template.md §9 是 tag 组合实例,不是演进记录,**不**作为 CHANGELOG 来源)
  - tag 命名规范:`v<MAJOR>.<MINOR>.<PATCH>`,与 plugin.json `version` 字段一致
  - release notes 必须包含:G1~G11 达成情况、AC-1~8 自测结果、手动验证清单 M-V1/M-V2 通过情况
...
**补丁登记(2026-09-03):阶段 A 版本号引用对齐现行冻结版 v0.5.5**

- **背景**:§阶段 A / §B1 / §D5 三处沿用早期 `0.4.0` 字面值,而 `src/prd.md` / `src/design.md` 现行冻结版本已是 **v0.5.5**(Round 15)。阶段 A 落地 `.claude-plugin/plugin.json` 时若照抄 `0.4.0`,会与三文档冻结版本及 §D5 "tag 与 plugin.json version 一致" 规范自相矛盾。
- **决议**:三处前向引用统一改 **0.5.5** —— §A1 `version: 0.5.5`、§B1 `<project>/raw/.aeps-plugin-version: 0.5.5`、§D5 `gh release create v0.5.5`(含 CHANGELOG 提炼措辞);§D5 release notes 的 `G1~G9` 同步改 **G1~G11**(G10 / G11 已于 v0.4.0 / v0.5.0 冻结)。
